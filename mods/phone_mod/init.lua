...
-- Register the phone item
minetest.register_craftitem("phone:smartphone", {
    description = "Smartphone",
    inventory_image = "default_steel_block.png", -- Use a default texture
    on_use = function(itemstack, user, pointed_thing)
        local player_name = user:get_player_name()
        minetest.show_formspec(player_name, "phone:interface", get_phone_formspec(player_name))
    end,
})
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    if formname == "phone:interface" and fields.send then
